                        ...
                        <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            ...
                            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                                <div className="flex">
                                    ...
                                    <div className="hidden bg-red-600 bg-blue-600 bg-black bg-white bg-green-600 bg-gray-600 bg-pink-600 bg-yellow-600 bg-purple-600"></div>
                                    <button className={`border-2 bg-${product.attributes.Colour}-600 bg-${product.attributes.Colour} border-gray-200 rounded-full w-6 h-6 focus:outline-none`}></button>
                                </div>
                                <div className="flex ml-6 items-center">
                                    <span className="mr-3">Size</span>
                                    <div className="relative">
                                        <select value={size} onChange={(e) => { setSize(e.target.value); }} id='select' className="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-500 text-base pl-3 pr-10">
                                            <option>{product.attributes.Size}</option>
                                            <option>{product.attributes.Size2}</option>
                                            <option>{product.attributes.Size3}</option>
                                            <option>{product.attributes.Size4}</option>
                                        </select>
                            ...
                            <div className="flex">
                                ...
                                <button onClick={() => { addToCart(product.attributes.Slug, 1, product.attributes.Name, product.attributes.Price, size, product.attributes.Colour, product.attributes.Image.data && product.attributes.Image.data.attributes.name) }} className="flex ml-auto text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Add To Cart</button>
...
export async function getServerSideProps(context) {
    ...
    let a = await fetch("http://localhost:1337/api/hooddies?filters[Slug]=" + context.query.slug + "&populate=*", { headers: headers })
