...
const hooddies = (props) => {
    return (
        <div>
            <Head>
                <title>Hooddies -- PlanetMarts</title>
            </Head>
            <div className="main">
                <section className="text-gray-600 body-font">
                    <div className="container px-5 py-24 mx-auto">
                        <div className="flex flex-wrap-reverse -my-4">
                            {props.product.data.map((item) => {
                                return (
                                    <Link href={`/hooddies/${item.attributes.Slug}`}>
...
export async function getServerSideProps(context) {
    ...
    let a = await fetch("http://localhost:1337/api/hooddies?populate=*", { headers: headers })
