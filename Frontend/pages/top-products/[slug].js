                        ...
                        <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                            ...
                            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
                                <div className="flex ml-2 items-center">
                                    <span className="mr-3">Quantity: {product.attributes.Quantity_Per_Kg_Gram} {product.attributes.Qauntity_or_Size}</span>
                                </div>
                            </div>
                            <div className="flex">
                                <span className="title-font font-medium text-2xl text-gray-900">₹{product.attributes.Price}</span>
                                <button onClick={() => { addToCart(product.attributes.Slug, 1, product.attributes.Name, product.attributes.Price, product.attributes.Category, product.attributes.Quantity_Per_Kg_Gram + product.attributes.Qauntity_or_Size, product.attributes.Image.data && product.attributes.Image.data.attributes.name) }} className="flex ml-auto text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded">Add To Cart</button>
...
export async function getServerSideProps(context) {
    ...
    let a = await fetch("http://localhost:1337/api/top-products?filters[Slug]=" + context.query.slug + "&populate=*", { headers: headers })
